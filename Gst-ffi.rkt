...
(define-gstreamer gst_init (_fun (_or-null _intptr) (_or-null (_ptr io (_list i _string))) -> _void))

;gboolean gst_init_check (int *argc, char **argv[], GError ** err);
(define-gstreamer gst_init_check (_fun _intptr (_ptr io (_ptr io (_list i _byte))) (_ptr io (_GError-pointer)) -> _gboolean))
...
(define-gstreamer gst_version (_fun _guint _guint _guint _guint -> _void))

;;gchar *             gst_version_string                  (void);
(define-gstreamer gst_version_string (_fun -> (_ptr o _string)))

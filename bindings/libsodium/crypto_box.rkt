...
(define CRYPTO/BOX/KEY/SIZE           (crypto/box/key/size))           ; Number of bytes in a public key.
(define CRYPTO/BOX/SECRET/SIZE        (crypto/box/secret/size))        ; Number of bytes in a secret key.
...
;; CRYPTO/BOX/KEY/SIZE and CRYPTO/BOX/SECRET/SIZE long respectively.
...
(define (crypto/box/keys)
  (let ((pk (make-bytes CRYPTO/BOX/KEY/SIZE    0))
        (sk (make-bytes CRYPTO/BOX/SECRET/SIZE 0)))
...
(define (crypto/box/encrypt m n pk sk)
  ; Input sanity.
;  (when (not (= (bytes-length pk) CRYPTO/BOX/KEY/SIZE))
;    (error 'crypto/box "incorrect public key length"))
;  (when (not (= (bytes-length sk) CRYPTO/BOX/SECRET/SIZE))

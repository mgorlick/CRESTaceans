...
(provide
 ...
 crypto/box/pre
 crypto/unbox/pre
 crypto/box/public-key-bytes
 crypto/box/secret-key-bytes
 crypto/box/nonce-bytes
 crypto/box/primitive)

;; Public/secret key encryption.

(define-sodium-constant crypto/box/public-key-bytes  "crypto_box_publickeybytes" _int) ; Size of a public key in bytes.
(define-sodium-constant crypto/box/secret-key-bytes  "crypto_box_secretkeybytes" _int) ; Size of a secret key in bytes.
(define-sodium-constant crypto/box/precompute-bytes  "crypto_box_beforenmbytes"  _int) ; Size in bytes of precomputed keying information.
(define-sodium-constant crypto/box/nonce-bytes       "crypto_box_noncebytes"     _int) ; Size of nonce in bytes.
(define-sodium-constant crypto/box/plain-zero-bytes  "crypto_box_zerobytes"      _int) ; Number of leading 0 bytes in a message (plaintext)
(define-sodium-constant crypto/box/cipher-zero-bytes "crypto_box_boxzerobytes"   _int) ; Number of leading 0 bytes in a ciphertext.
...
(define CRYPTO-BOX/PUBLIC-KEY-BYTES  (crypto/box/public-key-bytes))  ; Number of bytes in a public key.
(define CRYPTO-BOX/SECRET-KEY-BYTES  (crypto/box/secret-key-bytes))  ; Number of bytes in a secret key.
(define CRYPTO-BOX/PLAIN-ZERO-BYTES  (crypto/box/plain-zero-bytes))  ; Number of leading 0 bytes (padding) in a message.
(define CRYPTO-BOX/CIPHER-ZERO-BYTES (crypto/box/cipher-zero-bytes)) ; Number of leading 0 bytes (padding) in a ciphertext.
(define CRYPTO-BOX/NONCE-BYTES       (crypto/box/nonce-bytes))       ; Number of bytes in a nonce.
(define CRYPTO-BOX/PRECOMPUTE-BYTES  (crypto/box/precompute-bytes))  ; Number of bytes in precomputed public/secret shortcut.
...
;; CRYPTO-BOX/PUBLIC-KEY-BYTES and CRYPTO-BOX/SECRET-KEY-BYTES long respectively.
...
(define (crypto/box/keys)
  (let ((pk (make-bytes CRYPTO-BOX/PUBLIC-KEY-BYTES 0))
        (sk (make-bytes CRYPTO-BOX/SECRET-KEY-BYTES 0)))
    (if (zero? (crypto-box-key-pair pk sk))
        (values pk sk)
        (error 'crypto/box/keys "unable to generate a public/secret key pair"))))
...
(define (crypto/box m n pk sk)
  ; Input sanity.
  (when (not (= (bytes-length pk) CRYPTO-BOX/PUBLIC-KEY-BYTES))
    (error 'crypto/box "incorrect public key length"))
  (when (not (= (bytes-length sk) CRYPTO-BOX/SECRET-KEY-BYTES))
    (error 'crypto/box "incorrect secret key length"))
  (when (not (= (bytes-length n) CRYPTO-BOX/NONCE-BYTES))
    (error 'crypto/box "incorrect nonce length"))

  (let* ((m/pad/length (+ (bytes-length m) CRYPTO-BOX/PLAIN-ZERO-BYTES))
         (m/pad (make-bytes m/pad/length 0))
         (c     (make-bytes m/pad/length 0))) ; 
    (bytes-copy! m/pad CRYPTO-BOX/PLAIN-ZERO-BYTES m) ; Hence the first CRYPTO-BOX/PLAIN-ZERO-BYTES of m are 0.
    (when (not (zero? (crypto-box c m/pad m/pad/length n pk sk)))
      (error 'crypto/box "unable to box (encrypt) a message"))
    (subbytes c CRYPTO-BOX/CIPHER-ZERO-BYTES)))
...
(define (crypto/box/precompute pk sk)
  ...
  (let ((k (make-bytes CRYPTO-BOX/PRECOMPUTE-BYTES 0)))
...
(define (crypto/box/pre m nonce pre)
  ; Input sanity.
  (when (not (= (bytes-length nonce) CRYPTO-BOX/NONCE-BYTES))
    (error 'crypto/box/pre "incorrect nonce length"))  
  (when (not (= (bytes-length pre) CRYPTO-BOX/PRECOMPUTE-BYTES))
  ...
  (let* ((m/pad/length (+ (bytes-length m) CRYPTO-BOX/PLAIN-ZERO-BYTES))
    ...
    (subbytes ciphertext CRYPTO-BOX/CIPHER-ZERO-BYTES))) ; Retun the ciphertext.
...
(define (crypto/unbox/pre c n pre)
  ; Input sanity.
  (when (not (= (bytes-length n) CRYPTO-BOX/NONCE-BYTES))

...
(define CRYPTO/SIGN/KEY/SIZE    (crypto/sign/key/size))    ; Number of bytes in verification key.
(define CRYPTO/SIGN/SECRET/SIZE (crypto/sign/secret/size)) ; Number of bytes in signing key.
...
(define (crypto/sign/keys)
  (let ((pk (make-bytes CRYPTO/SIGN/KEY/SIZE    0))
        (sk (make-bytes CRYPTO/SIGN/SECRET/SIZE 0)))
...
;; Returns a signed message where the first CRYPTO/SIGN/SIZE are the signature and the rest are message m.
(define (crypto/sign m sk)
;  (when (not (= (bytes-length sk) CRYPTO/SIGN/SECRET/SIZE))
...
;  (when (not (= (bytes-length pk) CRYPTO/SIGN/KEY/SIZE))

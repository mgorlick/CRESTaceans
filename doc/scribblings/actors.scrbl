...
@defproc[(actor/new [chieftain actor?][nickname list?]) (actor? locative?)]{Creates a new actor whose clan chieftain is given returning the primal locative of the actor. There is only one primal locative per actor. List @racket[nickname] provides a name to identify the actor. An actor is an independent thread of computation that may transmit asynchronous messages to other actors, receive asynchronous messages from other actors, conduct private computation, and spawn new actors. It is implemented as a vector where the 1st element is @racket['actor].}
                                   

 @;{This definition is wrong -- don't know how to express a return value that is multiple values.}                                             
   @defproc[(actor/chieftain/new [chieftain actor?][nickname list?]) (actor? locative?)]{Creates a new chieftain by returning the primal locative of the chieftain. Chieftains are the only actors that are members of two clans: the parent clan that gave them birth and the clan that they themselves represent. In addition clan chieftains are the only actors for which their actor id and their clan id are identical. This function returns two values, a chieftain actor and its locative.}
                  
                                                       @;{This definition is wrong -- don't know how to express a return value that is multiple values.}                                                                
@defproc[(actor/root/new) (actor? locative?)]{Creates a @racket[ROOT] clan chieftain (only one must exist per island). The root chieftain is the only chieftain (and actor) on an island that does not have a superior chieftain. This function returns two values, the root actor and its locative.}                     

...
(define buffer (make-bytes 10000)) ; a buffer to reuse
(define (roundtrip pkt)
  (define written (packet->bytes pkt buffer))
  (bytes->packet buffer written))
(define (has-min-header? pkt)
  (define written (packet->bytes pkt buffer))
  (>= written 16))
...
(define-test-suite msg-serialization-tests

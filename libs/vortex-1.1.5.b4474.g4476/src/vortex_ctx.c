 ...
 *  Software Foundation, Inc., S59 Temple Place, Suite 330, Boston, MA
 *  02111-1307 USA
 *  
...
VortexMutex* vortex_ctx_get_frame_id_mutex (VortexCtx* ctx) {
  return ctx->frame_id_mutex;
}

VortexMutex* vortex_ctx_get_connection_id_mutex (VortexCtx* ctx) {
  return ctx->connection_id_mutex;
}

VortexMutex* vortex_ctx_get_listener_mutex (VortexCtx* ctx) {
  return ctx->listener_mutex;
}

VortexMutex* vortex_ctx_get_listener_unlock (VortexCtx* ctx) {
  return ctx->exit_mutex;
}

VortexMutex* vortex_ctx_get_exit_mutex (VortexCtx* ctx) {
  return ctx->exit_mutex;
}

VortexMutex* vortex_ctx_get_profiles_list_mutex (VortexCtx* ctx) {
  return ctx->profiles_list_mutex;
}

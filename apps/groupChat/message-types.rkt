...
(define-tuple-type (RegistryAdd) [param value])
(define-tuple-type (RegistryGet) [param])
(define-tuple-type (RegistryDel) [] [(curl = #f)])
(define-tuple-type (AddSubscriber) [curl])
(define-tuple-type (RemoveSubscriber) [] [(curl = #f)])
(define-tuple-type (ChatMsg) [nickname text timestamp])
(define-tuple-type (LocalMsg) [nickname text timestamp])
(define-tuple-type (SubscribeLocal) [thread])
(define-tuple-type (CreateChat) [host port groupname])
(define-tuple-type (JoinChat) [host port groupname])
(define-tuple-type (CP-Chat) [host port])
(define-tuple-type (MV-Chat) [host port])

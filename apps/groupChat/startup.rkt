...
(define (metadata->benv metadata)
  (cond [(meta-has-any? metadata is/proxy) CHAT-PROXY]
        ;[(meta-has-any? metadata accepts/webm) VIDEO-DECODE]
        ;[(meta-has-any? metadata produces/webm) VIDEO-ENCODE]        
        [(meta-has-any? metadata is/registry) CHAT-REGISTRY]
        [(meta-has-any? metadata is/endpoint) CHAT-ENDPOINT]
        [else CHAT-BASE]))
...
(define ISLAND-CURL@ "NO-CURL-YET")

(unless (argsassoc "--no-root")
  ...
  (actor/jumpstart ROOT (λ ()
                          (PROMISSARY ROOT)
                          (my-root-loop)))
  (define the-registry (registry))
  (curl/send PUBLIC-CURL (spawn/new the-registry (make-metadata is/registry (nick 'chat-registry)) #f))
)

(when (argsassoc "--no-root")        
  ;(define public-locative (locative/cons/any root-locative A-LONG-TIME A-LONG-TIME #t #t))
  ;(locative/id! public-locative '(public))
  ;(define PUBLIC-CURL (curl/new/any public-locative null #f))
  (set! ISLAND-CURL@ (curl/get-public (argsassoc "--vhost" #:no-val *LISTENING-ON* #:default *LISTENING-ON*)
...
(define server-where@ (curl/get-public (argsassoc "--vhost" #:no-val *LISTENING-ON* #:default *LISTENING-ON*)
                                       (argsassoc "--vport" #:call string->number 
                                                  #:no-val *LOCALPORT* 
                                                  #:default *LOCALPORT*)))

(define *CHATGROUP* (argsassoc "--chatgroupname" #:no-val "publicindex"))
...
(when (argsassoc "--new-chatgroup")
    
  ;(define init-server (newgroup+client-startup server-where@ *CHATGROUP* server-where@))
  (define init-server (server-startup server-where@ *CHATGROUP*))
  (curl/send ISLAND-CURL@ (spawn/new init-server (make-metadata (nick 'server-deployer)) #f))
  )
...
(unless (argsassoc "--no-gui")
  (sleep 1)
  (define init-gui (local-chatcontroller ISLAND-CURL@ *CHATGROUP* ))
  (curl/send ISLAND-CURL@ (spawn/new init-gui (make-metadata is/proxy (nick 'gui-deployer)) #f))
  
 ; (sleep 1)
  ;; create a second chat endpoint instance
 ; (curl/send ISLAND-CURL@ (spawn/new init-gui (make-metadata is/proxy (nick 'gui-deployer)) #f))
  )

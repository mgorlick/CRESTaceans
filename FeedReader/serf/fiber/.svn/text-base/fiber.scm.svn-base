...
(define this-dispatch ) ; Mailbox of dispatcher for message relays to/from external sites.
(define this-fiber    ) ; Object instance of this fiber.
(define this-mailbox  ) ; Mailbox assigned to this fiber.
(define this-uri      ) ; URI under which the fiber is executing.
(define this-log      ) ; The log (if any) for this fiber.
(define this-peer     ) ; The CREST peer under which this fiber is running.
...
(define (%fiber-default-error-handler awareness error-continuation)
  ...
  (let ((location (or (error-location awareness) "error location unknown"))
    ...
    (log/audit
     (this-log)
     (string-append (always-string location) ": " (always-string message)))
    #f))
...
(add-methods
 ...
 (list
  ...
  (method
   ((<fiber> f) (<procedure> thunk) (<mailbox> m) (<mailbox> dispatch))
   (:moniker! f #f)
   (:thunk!   f thunk)
   (:thread!  f 
    (parameterize
  ...
  (method
   ((<fiber> f) (<procedure> thunk) (<mailbox> m) (<mailbox> dispatch) (<value> peer))
  (:moniker! f #f)
  (:thunk!   f thunk)
  (:thread!  f 
    (parameterize
...
(add-methods
 ...
 (list
  ...
  (method ((<procedure> thunk) (<mailbox> m) (<mailbox> dispatch)) (make <fiber> thunk m dispatch))
  (method ((<procedure> thunk) (<mailbox> m) (<mailbox> dispatch) (<value> peer)) (make <fiber> thunk m dispatch peer))))


(set! this-dispatch  (make-parameter #f)) ; Mailbox of dispatcher for message relays to/from external sites.
(set! this-fiber     (make-parameter #f)) ; Object instance of this fiber.
(set! this-mailbox   (make-parameter #f)) ; Mailbox assigned to this fiber.
(set! this-uri       (make-parameter #f)) ; URI under which the fiber is executing.
(set! this-log       (make-parameter #f)) ; The Serf logger (if any) for this fiber.
(set! this-peer      (make-parameter #f)) ; The Serf peer under which the fiber is executing.

...
(: make-beep-message (Bytes Bytes Bytes Bytes -> beep-message))
(define (make-beep-message b1 b2 b3 b4)
  (beep-message b1 b2 b3 b4))
...
(define (payload->beep-message payload-bytes)
  (let ([opks (regexp-match OPK-rx payload-bytes)]
        [rpks (regexp-match RPK-rx payload-bytes)]
        [macs (regexp-match MAC-rx payload-bytes)]
        [bodys (regexp-match Body-rx payload-bytes)])
    (cond
      [(and (listof-bytes? bodys)
            (listof-bytes? opks)
            (listof-bytes? rpks)
            (listof-bytes? macs))
       (let ([opk (extract-field-value opks OPK-ele terminator-ele)]
...
(: mac-calculate/encode (Mac-Calculator Bytes Bytes -> Bytes))
...
(: message-encode ((U Bytes String) -> Bytes))
(define (message-encode msg)
  (cond
    [(bytes? msg) (base64-url-encode msg)]
    [(string? msg) (base64-url-encode (string->bytes/utf-8 msg))]))

(: message-validate/decode!? (Mac-Validator beep-message -> Boolean))
(define (message-validate/decode!? f m)
  (let ([understand? (message-mac-validate f m)])
    (when understand?
      (set-beep-message-body! m (base64-url-decode (beep-message-body m))))
    understand?))

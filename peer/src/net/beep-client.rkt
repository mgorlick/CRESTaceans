...
(define (make-beepcli amanager)
  (let ([context (new-ctx #f #f #f)])
    ...
    (beepcli context (make-hash) amanager)))
...
(connection*
 [context "localhost" "44000" on-connect on-connect-data]
 ...
 (vortex-sasl-set-propertie connection 'sasl-auth-id this-public-key-str #f)
 (vortex-sasl-set-propertie connection 'sasl-authorization-id remote-public-key-str #f)
...
(provide/contract
 ...
 [beepcli-connect (beepcli? string? clan? (or/c procedure? #f) any/c . -> . void)])
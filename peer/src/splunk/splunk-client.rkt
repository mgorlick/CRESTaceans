...
(define (make-splunk-client service-base-url username password)
  ...
  (define (login sc service-base username password)
    (let* ([response (query 
                      (POST-wwwform sc login-url `((username . ,username) 
                                                   (password . ,password)))
...
(define (get-search-results sc sid)
  (define job-results-url 
    (string-append (splunk-client-base-url sc)
                   search-jobs-path 
                   "/" sid "/results?output_mode=json"))
  (define sk (splunk-client-session-key sc))
  (query (GET sc job-results-url) 
         output-port->sjson
         "Error retrieving or decoding search results: "))
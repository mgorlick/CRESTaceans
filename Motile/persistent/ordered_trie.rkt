...
(define (counter/shared/new)
  (let ((counter 1) ; We'll count 1, 2, 3, ...
        ; Required since multiple thread may be inserting key/value pairs into one or more
        ; tries simultaneously. Sigh.
...
(define (trie/pairs/total x)
  (let loop ((entry x)
    ...
    (cond
      ...
      (else
       (error 'trie/count "unknown node type: ~s" entry)))))

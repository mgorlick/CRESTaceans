...
(define (<serf/imposter>)
  (let* ((self                (make-object))
	...
	(let ((authority (address/canonical host port)))
	  (set-attribute context :imposter.authority: (java-wrap authority))
	...
	(let ((authority (java-unwrap (get-attribute    context :imposter.authority:)))
	...
	(let* ((authority (java-unwrap (get-attribute context :imposter.authority:)))
	  ...
	  (display (format "submit-request:~a\n" (get-attribute context (->jstring "http.connection"))))
	  ...
	  (cond
	   (ticket
	    (let ((to       (:ticket-reply ticket))
		  ...
		  (metadata (:ticket-tag ticket)))
	      (! to body metadata)) ; Send the wrapped response back to the originator of the request.
	    (audit
	...
	(let* ((context   (get-context connection))
	       (authority (java-unwrap (get-attribute context :imposter.authority:))))
	...
	(let* ((context   (get-context connection))
	  ...
	  (audit self "connection/timeout" (format "~a context:~x" authority (java-object-to-id context))))))
	...
	(let* ((context   (get-context connection))
	       (authority (java-unwrap (get-attribute context :imposter.authority:))))
	  (hashtable/remove! connections authority) ; Rescind the connection.
	  (audit self "connection/closed" (format "~a context:~x" authority (java-object-to-id context))))))

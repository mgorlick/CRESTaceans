/*
 * $HeadURL: https://svn.apache.org/repos/asf/httpcomponents/httpcore/tags/4.0.1/httpcore-nio/src/examples/org/apache/http/examples/nio/NHttpServer.java $
 * $Revision: 744516 $
...
public class NioHttpServer {
    ...
    ListeningIOReactor io_reactor;
    int port;

    public NioHttpServer(
      int port,	// TCP port for the server.
      ...
      EventListener connection_listener) throws Exception {
        ...
        BufferingHttpServiceHandler service_handler = new BufferingHttpServiceHandler(
                httpproc,
        ...
        registry.register("*", request_responder);
	if (document_root != null) {
	    registry.register("/static/*", new HttpFileHandler(document_root));
	}

        service_handler.setHandlerResolver(registry); 
	...
	// Use two worker threads for the IO reactor.
        ...
        io_event_dispatch = new DefaultServerIOEventDispatch(service_handler, params);
	this.port = port; // Set the listening TCP port.
    }

    public void go() throws Exception { // Propagate exceptions to Scheme level.
	io_reactor.listen(new InetSocketAddress(port));

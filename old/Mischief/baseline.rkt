...
(define-syntax-rule (define/global/0 symbol procedure)
  (cons symbol (global/0/generate symbol procedure)))

(define-syntax-rule (define/global/1 symbol procedure)
  (cons symbol (global/1/generate symbol procedure)))

(define-syntax-rule (define/global/2 symbol procedure)
  (cons symbol (global/2/generate symbol procedure)))

(define-syntax-rule (define/global/3 symbol procedure)
  (cons symbol (global/3/generate symbol procedure)))

(define-syntax-rule (define/global/N symbol procedure)
  (cons symbol (global/N/generate symbol procedure)))

;; Special cases
;; Higher-order host primitives that accept a closure C as an argument, for example, apply or map,
;; require special treatment as a Mischief closure requires a continuation k as its first argument.

(define (k/RETURN x) x) ; The trivial continuation.

;; A Mischief-specific reworkings of map, apply, and for-each that accomodates the closure argument correctly.
(define (mischief/map rtk f . arguments)
  (if rtk
      (let ((g (lambda rest (apply f k/RETURN rest))))
        (rtk (apply map g arguments)))
      (vector 'reference/global 'map)))

(define (mischief/apply rtk f . arguments)
  (if rtk
      (rtk (apply f k/RETURN arguments))
      (vector 'reference/global 'apply)))

(define (mischief/for-each rtk f . arguments)
  (if rtk
      (let ((g (lambda rest (apply f k/RETURN rest))))
        (rtk (apply for-each g arguments)))
      (vector 'reference/global 'for-each)))

;(define (mischief/sort rtk items less?)
;  (if rtk
...
;; Mischief-specific reworkings of persistent vector primitives that accept functions as arguments.
;(define (mischief/vector/build rtk n f)
;  (if rtk
;      (let ((g (lambda (i) (f k/RETURN i))))
;        (rtk (vector/build n g)))
;      (vector 'global 'vector/build)))

(define mischief/vector/build
  (case-lambda
    ((rtk n f)
     (let ((g (lambda (i) (f k/RETURN i))))
        (rtk (vector/build n g))))
...
;(define (mischief/vector/fold/left rtk v f seed)
;  (if rtk
;      (let ((g (lambda (value seed) (f k/RETURN value seed))))
;        (rtk (vector/fold/left v g seed)))
;      (vector 'global 'vector/fold/left)))

(define mischief/vector/fold/left
  (case-lambda
    ((rtk v f seed)
     (let ((g (lambda (value seed) (f k/RETURN value seed))))
        (rtk (vector/fold/left v g seed))))
...
;(define (mischief/vector/fold/right rtk v f seed)
;  (if rtk
;      (let ((g (lambda (value seed) (f k/RETURN value seed))))
;        (rtk (vector/fold/right v g seed)))
;      (vector 'global 'vector/fold/right)))      

(define mischief/vector/fold/right
  (case-lambda
    ((rtk v f seed)
     (let ((g (lambda (value seed) (f k/RETURN value seed))))
        (rtk (vector/fold/right v g seed))))
...
;(define (mischief/vector/filter rtk v p)
;  (if rtk
;      (let ((q (lambda (value) (p k/RETURN value))))
;        (rtk (vector/filter v q)))
;      (vector 'global 'vector/filter)))

(define mischief/vector/filter
  (case-lambda
    ((rtk v p)
     (let ((q (lambda (value) (p k/RETURN value))))
       (rtk (vector/filter v q))))
...
;(define (mischief/vector/map rtk v f)
;  (if rtk
;      (let ((g (lambda (value) (f k/RETURN value))))
;        (rtk (vector/map v g)))
;      (vector 'global 'vector/map)))

(define mischief/vector/map
  (case-lambda
    ((rtk v f)
     (let ((g (lambda (value) (f k/RETURN value))))
        (rtk (vector/map v g))))
...
;; Mischief-specific rewworkings of higher-order, persistent hash table primitives 
;(define (mischief/hash/fold rtk h f seed)
;  (if rtk
;      (let ((g (lambda (value seed) (f k/RETURN value seed))))
;        (rtk (hash/fold h g seed)))
;      (vector 'global 'hash/fold)))

(define mischief/hash/fold
  (case-lambda
    ((rtk h f seed)
     (let ((g (lambda (value seed) (f k/RETURN value seed))))
        (rtk (hash/fold h g seed))))
...
;(define (mischief/hash/map rtk h f)
;  (if rtk
;      (let ((g (lambda (pair) (f k/RETURN pair))))
;        (rtk (hash/map h g)))
;      (vector 'global 'hash/map)))

(define mischief/hash/map
  (case-lambda
    ((rtk h f)
     (let ((g (lambda (pair) (f k/RETURN pair))))
        (rtk (hash/map h g))))
...
;(define (mischief/hash/filter rtk h p)
;  (if rtk
;      (let ((q (lambda (pair) (p k/RETURN pair))))
;        (rtk (hash/filter h q)))
;      (vector 'global 'hash/filter)))

(define mischief/hash/filter
  (case-lambda
    ((rtk h p)
     (let ((q (lambda (pair) (p k/RETURN pair))))
        (rtk (hash/filter h q))))
...
;(define (mischief/hash/partition rtk h p)
;  (if rtk
;      (let ((q (lambda (pair) (p k/RETURN pair))))
;        (rtk (hash/partition h q)))
;      (vector 'global 'hash/partition)))

(define mischief/hash/partition
  (case-lambda
    ((rtk h p)
     (let ((q (lambda (pair) (p k/RETURN pair))))
        (rtk (hash/partition h q))))
...
;; Mischief-specific reworkings of higher-order, persistent unordered set primitives.
;(define (mischief/set/fold rtk s f seed)
;  (if rtk
;      (let ((g (lambda (value seed) (f k/RETURN value seed))))
;        (rtk (set/fold s g seed)))
;      (vector 'global 'set/fold)))

(define mischief/set/fold
  (case-lambda
    ((rtk s f seed)
     (let ((g (lambda (value seed) (f k/RETURN value seed))))
        (rtk (set/fold s g seed))))
...
;(define (mischief/set/map rtk s f)
;  (if rtk
;      (let ((g (lambda (value) (f k/RETURN value))))
;        (rtk (set/map s g)))
;      (vector 'global 'set/map)))

(define mischief/set/map
  (case-lambda
    ((rtk s f)
     (let ((g (lambda (value) (f k/RETURN value))))
        (rtk (set/map s g))))
...
;(define (mischief/set/filter rtk s p)
;  (if rtk
;      (let ((q (lambda (value) (p k/RETURN value))))
;        (rtk (set/filter s q)))
;      (vector 'global 'set/filter)))

(define mischief/set/filter
  (case-lambda
    ((rtk s p)
     (let ((q (lambda (value) (p k/RETURN value))))
        (rtk (set/filter s q))))
...
;(define (mischief/set/partition rtk s p)
;  (display "inside mischief/set/partition\n")
;  (if rtk
;      (let ((q (lambda (value) (p k/RETURN value))))
;        (rtk (set/partition s q)))
;      (vector 'global 'set/partition)))

(define mischief/set/partition
  (case-lambda
    ((rtk s p)
     (let ((q (lambda (value) (p k/RETURN value))))
        (rtk (set/partition s q))))
...
;; The set of primitive procedures available to all Mischief mobile code.
(define BASELINE
  (make-immutable-hasheq
   (list
    ...
    (cons            'vector/build      mischief/vector/build)
    (cons            'vector/fold/left  mischief/vector/fold/left)
    (cons            'vector/fold/right mischief/vector/fold/right)
    ...
    (cons            'vector/filter     mischief/vector/filter)
    (cons            'vector/map        mischief/vector/map)
    (define/global/2 'vector/ref        vector/ref)
    ...
    (define/global/1 'hashpersist?      hashpersist?)
    ; Empty hash tables.
    ...
    (cons            'hash/fold         mischief/hash/fold)
    (cons            'hash/map          mischief/hash/map)
    (cons            'hash/filter       mischief/hash/filter)
    (cons            'hash/partition    mischief/hash/partition)
    ...
    ; Set combinators.
    (cons            'set/fold         mischief/set/fold)
    (cons            'set/map          mischief/set/map)
    (cons            'set/filter       mischief/set/filter)
    (cons            'set/partition    mischief/set/partition)

    ; Higher order functions.
    (cons            'apply     mischief/apply)
    (cons            'map       mischief/map)
    (cons            'for-each  mischief/for-each)

...
;; macros: an association list ((name . procedure) ...) of macro definitions where name is the syntactic keyword of the macro
;;   and procedure is its implementation

;; For example,
;;   (lambda (a b)
;;      (lambda (c d)
;;         (+ a c d)))
;; yields the lexical stack (#(#f (a b) () #f) (c d) (a) #f) where (#f (a b) () #f) is the bottom frame
;; containing two parameters (a b), no closed variables, and no macro definitions and the top frame contains parameters (c d),
;; a single closed variable a, and no macro definitions either in this frame or lower in the stack.
...
;(define (lexical/frame/make parameters closed) (vector #f parameters closed))
...
(define-syntax-rule (lexical/closed/set lexical closed) (vector-set! lexical 2 closed))
...
;; Returns #t if the syntactic keyword of e is the name of a macro definition and #f otherwise.
...
;; A frame with no parameters and no closed variables appearing within its scope is an empty frame.
(define (lexical/frame/empty? frame)
  (and
   (null? (lexical/frame/parameters frame))
   (null? (lexical/frame/closed frame))))

;; The frame contains parameters or closed variables or both.
...
(define (lambda/variables/closed e lexical)
  (let ((environ
         (lexical/push/parameters lexical (parameters/flat (lambda/parameters e)))))
    (foldl
     set/union null
...
;; prior is the immediately preceeding frame in the environment stack.
  ...
  (cond
    ...
    ((setter? e)
     (set/union
      ...
      (variables/closed (setter/value e) lexical)))
